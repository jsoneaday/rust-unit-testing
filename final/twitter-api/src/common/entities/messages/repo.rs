...
mod private_members {
    ...
    pub async fn insert_message_inner(conn: &Pool<Postgres>, user_id: i64, body: &str, group_type: i32, broadcasting_msg_id: Option<i64>) -> Result<i64, sqlx::Error> {
    ...
    pub async fn insert_response_message_inner(conn: &Pool<Postgres>, user_id: i64, body: &str, group_type: i32, original_msg_id: i64) -> Result<i64, sqlx::Error> {
        let tx = conn.begin().await.unwrap();

        let insert_result = sqlx::query_as::<_, EntityId>("insert into message (user_id, body, msg_group_type) values ($1, $2, $3) returning id")
            .bind(user_id)
            .bind(body)
            .bind(group_type)
            .fetch_one(conn)
        ...
        let insert_msg_response_result = sqlx::query_as::<_, EntityId>(
    ...
    pub async fn query_message_inner(conn: &Pool<Postgres>, id: i64) -> Result<Option<MessageWithFollowingAndBroadcastQueryResult>, sqlx::Error> {
        let message_result = sqlx::query_as::<_, MessageWithProfileQueryResult>(
            r"
        ...
        match message_result {
            Ok(message) => {
                if let Some(msg) = message {
                    let optional_matching_broadcast_message = get_broadcasting_message_of_message(conn, &msg).await;
                    let final_message = append_broadcast_msg_to_msg(optional_matching_broadcast_message.as_ref(), &msg);
    ...
    pub async fn query_messages_inner(conn: &Pool<Postgres>, user_id: i64, last_updated_at: DateTime<Utc>, page_size: i16) -> Result<Vec<MessageWithFollowingAndBroadcastQueryResult>, sqlx::Error> {
        ...
        match following_messages_with_profiles_result {
            Ok(following_messages) => {
                ...
                let final_message_list = append_broadcast_msgs_to_msgs(&optional_matching_broadcast_messages, following_messages);
    ...
    async fn get_broadcasting_messages_of_messages(
        conn: &Pool<Postgres>, following_messages_with_broadcasts: &Vec<MessageWithProfileQueryResult>
    ...
    async fn get_broadcasting_message_of_message(
        conn: &Pool<Postgres>, message: &MessageWithProfileQueryResult
    ...
    fn append_broadcast_msgs_to_msgs(
        optional_broadcast_messages: &Option<Vec<MessageWithProfileQueryResult>>, following_messages_with_broadcasts: Vec<MessageWithProfileQueryResult>
        ...
        following_messages_with_broadcasts
            ...
            .for_each(|following_message_with_broadcast| {
                ...
                final_list_of_messages.push(append_broadcast_msg_to_msg(matching_broadcast_msg, following_message_with_broadcast));
            });
...
impl InsertMessageFn for DbRepo {
    async fn insert_message(&self, conn: &Pool<Postgres>, user_id: i64, body: &str, group_type: i32, broadcasting_msg_id: Option<i64>) -> Result<i64, sqlx::Error> {
        // private_members::append_broadcast_msgs_to_msgs(&None, vec![]);
...
impl QueryMessagesFn for DbRepo {
    async fn query_messages(&self, conn: &Pool<Postgres>, user_id: i64, last_updated_at: DateTime<Utc>, page_size: i16) -> Result<Vec<MessageWithFollowingAndBroadcastQueryResult>, sqlx::Error> {
        private_members::query_messages_inner(conn, user_id, last_updated_at, page_size).await
    }
}
...
mod tests {
    ...
    async fn setup_data(conn: Pool<Postgres>) -> Fixtures {
        ...
        let profile = db_repo.insert_profile(&conn, profile_create.clone()).await;
        let profile_id = profile.unwrap();
        let original_msg_id = db_repo.insert_message(&conn, profile_id, "Testing body 123", PUBLIC_GROUP_TYPE, None).await.unwrap();
    ...
    mod test_mod_query_messages_by_following {
        ...
        async fn setup() -> QueryMsgFollowingFixtures {
            ...
            let mut following_users_messages: Vec<MessageWithFollowingAndBroadcastQueryResult> = Vec::new();
            ...
            for l in following_letters {
                ...
                let following_user_message_1_id = db_repo.insert_message(&conn, following_id, format!("Message {}: 1", l).as_str(), PUBLIC_GROUP_TYPE, None).await.unwrap();                
                ...
                let following_user_message_2_id = db_repo.insert_message(&conn, following_id, format!("Message {}: 2", l).as_str(), PUBLIC_GROUP_TYPE, None).await.unwrap();
        ...
        #[async_trait]
        impl QueryMessagesFn for QueryMsgFollowingDbRepo {
            async fn query_messages(&self, conn: &Pool<Postgres>, user_id: i64, last_updated_at: DateTime<Utc>, page_size: i16) -> Result<Vec<MessageWithFollowingAndBroadcastQueryResult>, sqlx::Error> {
                private_members::query_messages_inner(conn, user_id, last_updated_at, page_size).await
            }
        }
        ...
        impl InsertMessageFn for QueryMsgFollowingDbRepo {
            async fn insert_message(&self, _: &Pool<Postgres>, user_id: i64, body: &str, _: i32, _: Option<i64>) -> Result<i64, sqlx::Error> {                
        ...
        async fn test_query_messages_by_following() {
            ...
            for l in ["a", "b"] {
                ...
                let following_message_1_id = db_repo.insert_message(&conn, following_id, format!("Message {}: 1", l).as_str(), PUBLIC_GROUP_TYPE, None).await.unwrap();
                created_following_messages.push(following_message_1_id);
                let following_message_2_id = db_repo.insert_message(&conn, following_id, format!("Message {}: 2", l).as_str(), PUBLIC_GROUP_TYPE, None).await.unwrap();
            ...
            let following_messages = db_repo.query_messages(conn, follower_id, Utc::now(), 10).await.unwrap();
            let following_msg_ids = following_messages.iter().map(|fm| {
                fm.id
            }).collect::<Vec<i64>>();
